...
- 正常扣费流程不允许扣成负数。
- 用户端简化流水只展示时间、积分变动或消耗积分、关联作品；完整 usage 和倍率快照仅管理端展示。
...
| :----------- | :---------------- | :------------------------- | :-------------------------- | :---------------- | :--- |
| `DeepSeek-v4-flash` | `deepseek-v4-flash` | `0.11` | `0.11` | `0.22` | 输入成本 `1 元 / 100万 token`、输出成本 `2 元 / 100万 token`，按 10% 加价率生成。 |
| `DeepSeek-v4-pro` | `deepseek-v4-pro` | `0.02` | `1.32` | `2.64` | 缓存命中输入成本 `0.10 元 / 100万 token`，未命中输入成本 `12 元 / 100万 token`，输出成本 `24 元 / 100万 token`，按 10% 加价率生成。 |
