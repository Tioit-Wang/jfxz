...
erDiagram
    ...
    ai_models {
        ...
        decimal cache_hit_input_multiplier
        decimal cache_miss_input_multiplier
        decimal output_multiplier
        varchar status
    }
    ...
    plans {
        uuid id PK
        varchar name
        decimal monthly_points
        decimal bundled_topup_points
    }
    ...
    point_accounts {
        uuid id PK
        uuid user_id FK
        decimal monthly_points_balance
        decimal topup_points_balance
    }
...
- `ai_models` 保存平台模型目录与计费倍率，历史积分流水通过模型快照保留当时计费依据。
...
模型调用统一口径见 `管理端需求文档/模型管理需求文档.md`；数据库仅保存模型目录、平台调用 ID、展示信息、状态和计费倍率，不保存 API key。
...
| `cache_hit_input_multiplier`      | `decimal(8,2)`  | 是   | 缓存命中输入 token 计费倍率                                  |
| `cache_miss_input_multiplier`     | `decimal(8,2)`  | 是   | 缓存未命中输入 token 计费倍率                                |
| `output_multiplier`               | `decimal(8,2)`  | 是   | 输出 token 计费倍率                                          |
...
| `monthly_points`        | `decimal(12,2)` | 是   | 月度积分                             |
| `bundled_topup_points`  | `decimal(12,2)` | 是   | 套餐附带加油包积分                   |
| `status`                | `varchar(20)`   | 是   | 状态，建议值：`active`、`inactive`   |
| `sort_order`            | `integer`       | 否   | 排序值                               |
| `created_at`            | `timestamptz`   | 是   | 创建时间                             |
| `updated_at`            | `timestamptz`   | 是   | 更新时间                             |
...
| `id`                     | `uuid`        | 是   | 主键                         |
| `user_id`                | `uuid`        | 是   | 所属用户 ID，关联 `users.id` |
| `monthly_points_balance` | `decimal(12,2)` | 是 | 月度积分余额，保留 2 位小数 |
| `topup_points_balance`   | `decimal(12,2)` | 是 | 加油包积分余额，保留 2 位小数 |
| `updated_at`             | `timestamptz` | 是   | 更新时间                     |
...
| `change_type`                           | `varchar(20)`   | 是   | 变更类型，建议值：`grant`、`consume`、`expire`、`refund`、`adjust`、`system_grant` |
| `source_type`                           | `varchar(50)`   | 是   | 来源类型，例如 `plan_monthly`、`plan_bundled_topup`、`topup_pack`、`ai_chat`、`editor_check`、`system_overage_grant` |
| `source_id`                             | `uuid`          | 否   | 来源记录 ID                                                  |
| `related_transaction_id`                | `uuid`          | 否   | 关联流水 ID，用于系统赠送补平追溯对应消费流水                 |
...
| `cache_hit_input_multiplier_snapshot`   | `decimal(8,2)`  | 否   | 缓存命中输入倍率快照                                         |
| `cache_miss_input_multiplier_snapshot`  | `decimal(8,2)`  | 否   | 缓存未命中输入倍率快照                                       |
| `output_multiplier_snapshot`            | `decimal(8,2)`  | 否   | 输出倍率快照                                                 |
...
- `prompt_cache_hit_tokens` 使用缓存命中输入倍率。
- `prompt_cache_miss_tokens` 使用缓存未命中输入倍率。
- `completion_tokens` 使用输出倍率，`reasoning_tokens` 不重复计费。
- 若供应商未返回缓存拆分字段，输入 token 默认计入 `prompt_cache_miss_tokens`。
- 一次 AI 扣费可拆成多条 `consume` 流水：先扣 `monthly`，不足部分再扣 `topup`。
- 若剩余扣费超过当前加油包积分，`topup` 可在本次消费流水中临时扣为负数，随后追加 `system_grant` 流水补平到 0。
- `system_grant` 流水的 `bucket_type` 建议为 `topup`，`related_transaction_id` 指向导致超额的消费流水。
...
| display_name | provider_model_id | cache_hit_input_multiplier | cache_miss_input_multiplier | output_multiplier | 说明 |
| :----------- | :---------------- | :------------------------- | :-------------------------- | :---------------- | :--- |
| `DeepSeek-v4-flash` | `deepseek-v4-flash` | `0.11` | `0.11` | `0.22` | 输入成本 `1 元 / 100万 token`、输出成本 `2 元 / 100万 token`，按 10% 毛利生成。 |
| `DeepSeek-v4-pro` | `deepseek-v4-pro` | `0.02` | `1.32` | `2.64` | 缓存命中输入成本 `0.10 元 / 100万 token`，未命中输入成本 `12 元 / 100万 token`，输出成本 `24 元 / 100万 token`，按 10% 毛利生成。 |
